...
pkmEXTAudioFileReader::~pkmEXTAudioFileReader() { close(); }

bool pkmEXTAudioFileReader::open(string mPath, int sampleRate, int channels) {
  OSStatus err = noErr;
  //  const char *filename = basename((char *) mPath.c_str());

  CFURLRef mURL = CFURLCreateFromFileSystemRepresentation(
      NULL, (const UInt8 *)mPath.c_str(), mPath.size(), false);

  err = ExtAudioFileOpenURL(mURL, &mSoundID);
  CFRelease(mURL);
  mURL = NULL;
  if (err != noErr) {
          printf("[pkmEXTAudioFileReader]: ExtAudioFileOpenURL %s: err %d (file: '%s')\n", mPath.c_str(), (int) err, mPath.c_str());
    return false;
  }
  ...
  err =
      ExtAudioFileGetProperty(mSoundID, kExtAudioFileProperty_FileLengthFrames,
  ...
  err = ExtAudioFileGetProperty(mSoundID, kExtAudioFileProperty_FileDataFormat,
                                &propertySize, &format);
  if (err != noErr) {
  ...
  ExtAudioFileGetProperty(mSoundID, kExtAudioFileProperty_FileDataFormat,
                          &propSize, &dstFormat);

  dstFormat.mSampleRate = sampleRate;
  dstFormat.mFormatID = kAudioFormatLinearPCM;
  dstFormat.mBitsPerChannel = sizeof(float) * 8;
  dstFormat.mChannelsPerFrame = channels;  // set this to 2 for stereo
  ...
  err = ExtAudioFileSetProperty(
      mSoundID, kExtAudioFileProperty_ClientDataFormat, propSize, &dstFormat);
  if (err != noErr) {
...
void pkmEXTAudioFileReader::close() {
  if (loaded) ExtAudioFileDispose(mSoundID);
}

bool pkmEXTAudioFileReader::read(float *target, long start, long count,
                                 int sampleRate) {
  ...
  while (offset < count) {
    ...
    buf.mBuffers[0].mDataByteSize = (count - offset) *
                                    dstFormat.mChannelsPerFrame *
                                    (unsigned)(dstFormat.mBitsPerChannel / 8);
